...
D1<-read.csv(file ="timeEvalRF.csv",header = TRUE)
D2<-read.csv(file ="timeEvalCNN.csv",header = FALSE)
D3<-read.csv(file ="timeEvalRFSpect.csv",header = TRUE)
D1<-D1[,D1!=0]
D3<-D3[,D3!=0]
...
GRAPH<-ggplot(MM, aes(x = V2, y = V1))  +
  ...
  scale_y_continuous(name = "Execution times (Seconds)") +
  scale_x_discrete(name = "",limits=c("1","3","2"),labels=c("1"="RF on  features (STFT,MFCC,ZCR,CWT)","3"="RF on features (STFT only)","2"="CNN")) +
